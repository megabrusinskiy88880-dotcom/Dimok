...
if (UPLOAD_DIR !== defaultUploads && fs.existsSync(defaultUploads)) {
  fs.readdirSync(defaultUploads).forEach(file => {
    const destFile = path.join(UPLOAD_DIR, file);
    if (!fs.existsSync(destFile)) {
      fs.copyFileSync(path.join(defaultUploads, file), destFile);
    }
  });
}
